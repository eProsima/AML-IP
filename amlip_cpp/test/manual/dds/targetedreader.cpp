        ...
        char** argv)
{
...

    {
        ...
        logUser(AMLIPCPP_MANUAL_TEST, "Created Participant: " << participant << ". Creating Writer to send Id...");

        // Create Writer to send id (it must be transient local and reliable)
        eprosima::fastdds::dds::DataWriterQos qos;
        qos = eprosima::amlip::dds::Writer<eprosima::amlip::types::AmlipIdDataType>::default_datawriter_qos();
        ...
        logUser(AMLIPCPP_MANUAL_TEST, "Created Writer. Sending Id...");

        // Send own id
        eprosima::amlip::types::AmlipIdDataType own_id = participant.id();
        writer->publish(own_id);

        logUser(AMLIPCPP_MANUAL_TEST, "Writer sent data: " << own_id << ". Creating Targeted Reader...");
        ...
        logUser(AMLIPCPP_MANUAL_TEST, "Created Targeted Reader. Waiting data...");
